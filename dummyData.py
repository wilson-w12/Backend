...
teacher_data = []
# Create 5 to 8 teachers for each subject
for subject in subjects:
    ...
    for i in range(num_teachers):
        teacher_data.append({
            ...
            "password": bcrypt.hashpw(f"password{len(teacher_data)+1}".encode('utf-8'), bcrypt.gensalt()).decode('utf-8'),  
            "phone": generate_phone_number(),  
            "subjects": random.sample(subjects, random.randint(1, 2))  # 1 or 2 subjects
        })
...
for year in range(8, 13):  
    for set_letter in 'ABCDEFGHIJ':  
        num_students = random.randint(20, 30)  # Number of students in set
        for _ in range(num_students):
            # Generate random target grades for each student
            ...
            student_data.append({
                ...
                "target_grades": target_grades  # Assign target grades to student
            })
    ...
    for year in range(8, 13):  
        for set_letter in 'ABCDEFGHIJ': 
            for subject in subjects:  
                ...
                # Find students for year and set
...
if len(test_teachers) != 2:
    ...
    # Assign test teachers to 20 random classes 
    for test_teacher in test_teachers:
        assigned_classes = random.sample(list(db.classes.find()), 20)  # Randomly select 20 classes
        
        for cls in assigned_classes:
            # Ensure teacher is added to the class
            db.classes.update_one(
                {"_id": cls["_id"]},
                {"$addToSet": {"teacher_ids": test_teacher["_id"]}}  # Prevent duplicates
            )
# Print confirmation
